...
- Throw `MyError` with status 401 if token is missing or invalid
- Handle errors appropriately and throw `MyError` for any verification failures
...
- Throw `MyError` with status 403 if user doesn't have required role
...
- Add `signUp` function:
  - Check if user with email already exists (throw `MyError` if exists)
...
- Add `signIn` function:
  ...
  - Throw `MyError` with status 401 if email or password is invalid (use generic message: "Invalid email or password")
...
- Update `updatePostById` function:
  ...
  - Throw `MyError` with status 403 if user is not the author
...
- **Important:** Services should check ownership and throw `MyError` for authorization errors, but return `null` for "not found" cases
...
- Handle "not found" cases by throwing `MyError` with status 404 when service returns `null`
